 ...
 * Libraries
 * JIA JI KE JI
 * Create by Cuikai 2015-11-30
 */
class im{
    private $appKey = 'sfci50a7czd3i';                //appKey
    private $appSecret = 'VXsA425oABBqj';             //secret
...
//************************************接口二次封装 spirit***************************
    ...
    /**
     * 
     * 将若干用户加入指定组，用户将可以收到该群的消息。
     * @param $userIds           要加入群的用户 Id数组。（必传）
     * @param $groupId          要加入的群 Id。（必传）
     * @param $groupName        要加入的群 Id 对应的名称。（可选）
     * @return json|xml
     */
    public function groupJoinBatch($userIds, $groupId, $groupName) {
